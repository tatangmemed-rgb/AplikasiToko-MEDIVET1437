...
private fun StoreApp() {
    ...
    var transactionFilter by remember { mutableStateOf("Semua") }
    ...
    Scaffold(
        ...
        bottomBar = {
            NavigationBar {
                ...
                NavigationBarItem(page == 1, { page = 1; transactionFilter = "Semua" }, { Icon(Icons.Default.List, null) }, label = { Text("Transaksi") })
        ...
        when (page) {
            0 -> Home(entries, balances, padding, onAdd = { editing = null; pendingProofPath = ""; pendingProofToken = UUID.randomUUID().toString(); dialog = true }, onTransfer = { transferDialog = true }, onImport = { importLauncher.launch(arrayOf("text/csv", "text/comma-separated-values", "application/csv", "application/vnd.ms-excel", "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet", "text/plain")) }, onExport = { exportDialog = true }, onBackupRestore = { backupDialog = true }, onViewUnpaid = { transactionFilter = "Nota Belum Terbayar"; page = 1 }, onViewPaid = { transactionFilter = "Nota Terbayar"; page = 1 })
            1 -> TransactionPage(entries, padding, initialFilter = transactionFilter, onEdit = { editing = it; pendingProofPath = it.proofPath; pendingProofToken = UUID.randomUUID().toString(); if (it.type == "Transfer") transferDialog = true else dialog = true }, onDelete = { e -> save(entries.filterNot { it.id == e.id }) }, onViewProof = { openProof(context, it) })
...
@Composable
private fun Home(entries: List<Entry>, balances: Map<String, Long>, padding: PaddingValues, onAdd: () -> Unit, onTransfer: () -> Unit, onImport: () -> Unit, onExport: () -> Unit, onBackupRestore: () -> Unit, onViewUnpaid: () -> Unit, onViewPaid: () -> Unit) {
    val income = entries.filter { it.type == "Pemasukan" }.sumOf { it.amount }
    val expense = entries.filter { it.type == "Pengeluaran" || it.type == "Nota Terbayar" }.sumOf { it.amount }
    val unpaidNotes = entries.filter { it.type == "Nota Masuk" }
    ...
    LazyColumn(Modifier.fillMaxSize().padding(padding).padding(horizontal = 16.dp), verticalArrangement = Arrangement.spacedBy(12.dp), contentPadding = PaddingValues(vertical = 16.dp)) {
        item {
            ...
            Text("Kelola Kas Toko dan dua rekening bank.", color = Color.Gray)
        }
        item {
        ...
        item {
            NoteSummaryCard(
                title = "NOTA BELUM TERBAYAR",
                count = unpaidNotes.size,
                amount = unpaidTotal,
        ...
        item {
            Text("Aksi Cepat", fontSize = 21.sp, fontWeight = FontWeight.Bold)
        }
        item {
            Row(Modifier.fillMaxWidth(), horizontalArrangement = Arrangement.spacedBy(10.dp)) {
        ...
        item {
            Card(Modifier.fillMaxWidth()) {
                Column(Modifier.padding(16.dp)) {
                    Text("Ringkasan Aktivitas", fontWeight = FontWeight.Bold)
                    Text("Pemasukan  ${formatMoney(income)}", color = Color(0xFF159957))
                    Text("Pengeluaran ${formatMoney(expense)}", color = Color(0xFFD64545))
                    Text("Transfer tidak dihitung sebagai pemasukan/pengeluaran.", fontSize = 12.sp, color = Color.Gray)
                }
            }
        }
        if (entries.isEmpty()) item { Card(Modifier.fillMaxWidth()) { Text("Belum ada data transaksi. Saldo akan mengikuti data yang Anda masukkan.", Modifier.padding(20.dp), color = Color.Gray) } }
    }
}
...
private fun NoteSummaryCard(
    ...
    Card(
        ...
        Row(
            ...
            Icon(icon, null, tint = Color(0xFF159957), modifier = Modifier.size(32.dp))
            Spacer(Modifier.width(12.dp))
            Column(Modifier.weight(1f)) {
                Text(title, fontSize = 14.sp, fontWeight = FontWeight.Bold)
                Text(
                    if (count == 0) "Belum ada nota" else "$count nota",
                    fontSize = 13.sp,
                    color = Color.Gray
                )
            }
            Column(horizontalAlignment = androidx.compose.ui.Alignment.End) {
                Text(formatMoney(amount), fontSize = 18.sp, fontWeight = FontWeight.Bold)
                Text("Lihat ›", fontSize = 12.sp, color = Color(0xFF159957))
            }
        }
    }
}
...
@Composable
private fun TransactionPage(entries: List<Entry>, padding: PaddingValues, initialFilter: String = "Semua", onEdit: (Entry) -> Unit, onDelete: (Entry) -> Unit, onViewProof: (String) -> Unit) {
    ...
    val filtered = when (filter) {
        ...
        "Nota Belum Terbayar" -> entries.filter { it.type == "Nota Masuk" }
    ...
    LazyColumn(Modifier.fillMaxSize().padding(padding).padding(horizontal = 16.dp), verticalArrangement = Arrangement.spacedBy(8.dp), contentPadding = PaddingValues(vertical = 16.dp)) {
        item {
            ...
            Text("Koreksi, lihat bukti, hapus data, atau cek perpindahan saldo.", color = Color.Gray)
            Row(Modifier.horizontalScroll(rememberScrollState()).padding(vertical = 10.dp), horizontalArrangement = Arrangement.spacedBy(6.dp)) {
                listOf("Semua", CASH, BANK1, BANK2, "Nota Belum Terbayar", "Nota Terbayar").forEach { FilterChip(filter == it, { filter = it }, label = { Text(it) }) }
            }
        }
            ...
            Card(Modifier.fillMaxWidth()) {
                Column(Modifier.padding(14.dp)) {
                    Row(Modifier.fillMaxWidth(), horizontalArrangement = Arrangement.SpaceBetween) {
                        ...
                        Text(if (entry.type == "Transfer") formatMoney(entry.amount) else formatMoney(entry.amount), fontWeight = FontWeight.Bold, color = if (entry.type == "Pemasukan") Color(0xFF159957) else if (entry.type == "Transfer") Color(0xFF1565C0) else Color(0xFFD64545))
                    }
...
private fun ReportPage(entries: List<Entry>, balances: Map<String, Long>, padding: PaddingValues) {
    ...
    LazyColumn(Modifier.fillMaxSize().padding(padding).padding(16.dp), verticalArrangement = Arrangement.spacedBy(10.dp), contentPadding = PaddingValues(bottom = 20.dp)) {
        item { Text("Laporan Keuangan", fontSize = 25.sp, fontWeight = FontWeight.Bold); Text("Ringkasan seluruh sumber dana.", color = Color.Gray) }
        ...
        item { SummaryCard("Total Pemasukan", income, Color(0xFF159957)) }
        item { SummaryCard("Total Pengeluaran", expense, Color(0xFFD64545)) }
        item { SummaryCard("Selisih", income - expense, Color(0xFF1565C0)) }
        item { Text("Jumlah transaksi: ${entries.size}", color = Color.Gray) }
    }
}
...
private fun convertRowsToEntries(rows: List<List<String>>): List<Entry> {
    ...
    val dateIdx = indexOfAny("tanggal", "date"); val typeIdx = indexOfAny("jenis", "type", "tipe", "kategori", "category"); val amountIdx = indexOfAny("nominal", "amount", "jumlah", "total", "harga"); val noteIdx = indexOfAny("keterangan", "catatan", "note", "deskripsi", "description", "nama"); val accountIdx = indexOfAny("akun", "account", "rekening", "sumber")
    ...
    rows.drop(start).forEachIndexed { idx, row ->
        ...
        val type = when { typeRaw.contains("transfer") -> "Transfer"; typeRaw.contains("pengeluaran") || typeRaw.contains("expense") || typeRaw.contains("keluar") -> "Pengeluaran"; typeRaw.contains("nota") && (typeRaw.contains("bayar") || typeRaw.contains("paid")) -> "Nota Terbayar"; typeRaw.contains("nota") -> "Nota Masuk"; else -> "Pemasukan" }
        val note = if (noteIdx >= 0 && noteIdx < row.size && row[noteIdx].isNotBlank()) row[noteIdx] else "Impor data"
        val accountRaw = if (accountIdx >= 0 && accountIdx < row.size) row[accountIdx].lowercase(Locale.ROOT) else ""
        val account = when { accountRaw.contains("bank 2") || accountRaw.contains("bank2") -> BANK2; accountRaw.contains("bank 1") || accountRaw.contains("bank1") -> BANK1; else -> CASH }
        result += Entry(System.currentTimeMillis() + idx, date, type, amount, note, account = account)
    }
    return result
}
...
private fun buildWhatsAppReport(entries: List<Entry>, balances: Map<String, Long>): String {
    ...
    return buildString {
        ...
        append("SALDO\n")
        append("Kas Toko: ${formatMoney(balances[CASH] ?: 0)}\n")
        append("Bank 1: ${formatMoney(balances[BANK1] ?: 0)}\n")
        append("Bank 2: ${formatMoney(balances[BANK2] ?: 0)}\n")
        append("Total Saldo: ${formatMoney(balances.values.sum())}\n\n")
        append("AKTIVITAS\n")
        append("Pemasukan: ${formatMoney(income)}\n")
        append("Pengeluaran: ${formatMoney(expense)}\n")
        append("Selisih: ${formatMoney(income - expense)}\n")
        append("Transfer antar saldo: ${entries.count { it.type == "Transfer" }} transaksi\n")
        append("Jumlah transaksi: ${entries.size}\n")
    }
}
...
private fun writePdf(context: Context, uri: Uri, entries: List<Entry>, balances: Map<String, Long>) {
    ...
    fun line(text: String) { if (y > 800f) { pdf.finishPage(page); pageNumber++; page = pdf.startPage(android.graphics.pdf.PdfDocument.PageInfo.Builder(pageWidth, pageHeight, pageNumber).create()); canvas = page.canvas; y = 40f }; canvas.drawText(text.take(95), 32f, y, paint); y += 20f }
    line("LAPORAN KEUANGAN TOKO"); y += 8f
    line("Kas Toko: ${formatMoney(balances[CASH] ?: 0)}"); line("Bank 1: ${formatMoney(balances[BANK1] ?: 0)}"); line("Bank 2: ${formatMoney(balances[BANK2] ?: 0)}"); line("Total Saldo: ${formatMoney(balances.values.sum())}"); y += 8f
    entries.forEach { e -> val detail = if (e.type == "Transfer") "${e.fromAccount} -> ${e.toAccount}" else e.account; line("${e.date} | ${e.type} | $detail | ${formatMoney(e.amount)} | ${e.note}") }
...
private fun entryToJson(e: Entry): JSONObject = JSONObject().apply { put("id", e.id); put("date", e.date); put("type", e.type); put("amount", e.amount); put("note", e.note); put("proofName", if (e.proofPath.isNotBlank()) File(e.proofPath).name else ""); put("account", e.account); put("fromAccount", e.fromAccount); put("toAccount", e.toAccount) }
...
private fun loadEntries(context: Context): List<Entry> {
    ...
    return buildList { for (i in 0 until a.length()) { val o = a.getJSONObject(i); add(Entry(o.getLong("id"), o.getString("date"), o.getString("type"), o.getLong("amount"), o.getString("note"), o.optString("proofPath", ""), o.optString("account", CASH), o.optString("fromAccount", ""), o.optString("toAccount", ""))) } }
}
private fun saveEntries(context: Context, entries: List<Entry>) { val a = JSONArray(); entries.forEach { a.put(JSONObject().apply { put("id", it.id); put("date", it.date); put("type", it.type); put("amount", it.amount); put("note", it.note); put("proofPath", it.proofPath); put("account", it.account); put("fromAccount", it.fromAccount); put("toAccount", it.toAccount) }) }; context.getSharedPreferences("finance", Context.MODE_PRIVATE).edit().putString("entries", a.toString()).apply() }
