    ...
    LazyColumn(Modifier.fillMaxSize().padding(padding).padding(horizontal = 16.dp), verticalArrangement = Arrangement.spacedBy(8.dp), contentPadding = PaddingValues(vertical = 16.dp)) {
        ...
        if (filtered.isEmpty()) item {
            Card(Modifier.fillMaxWidth()) {
                Text(
                    if (filter == "Pengeluaran") "Belum ada pengeluaran yang diinput." else if (filter == "Nota Belum Terbayar") "Tidak ada nota yang belum terbayar." else if (filter == "Nota Terbayar") "Belum ada nota terbayar." else "Belum ada transaksi.",
