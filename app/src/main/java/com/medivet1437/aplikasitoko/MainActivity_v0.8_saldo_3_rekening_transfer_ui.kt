...
private fun StoreApp() {
    ...
    Scaffold(
        ...
        when (page) {
            0 -> Home(entries, balances, padding, onAdd = { editing = null; pendingProofPath = ""; pendingProofToken = UUID.randomUUID().toString(); dialog = true }, onTransfer = { transferDialog = true }, onImport = { importLauncher.launch(arrayOf("text/csv", "text/comma-separated-values", "application/csv", "application/vnd.ms-excel", "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet", "text/plain")) }, onExport = { exportDialog = true }, onBackupRestore = { backupDialog = true })
            1 -> TransactionPage(entries, padding, onEdit = { editing = it; pendingProofPath = it.proofPath; pendingProofToken = UUID.randomUUID().toString(); if (it.type == "Transfer") transferDialog = true else dialog = true }, onDelete = { e -> save(entries.filterNot { it.id == e.id }) }, onViewProof = { openProof(context, it) })
...
@Composable
private fun Home(entries: List<Entry>, balances: Map<String, Long>, padding: PaddingValues, onAdd: () -> Unit, onTransfer: () -> Unit, onImport: () -> Unit, onExport: () -> Unit, onBackupRestore: () -> Unit) {
...
private fun TransactionPage(entries: List<Entry>, padding: PaddingValues, onEdit: (Entry) -> Unit, onDelete: (Entry) -> Unit, onViewProof: (String) -> Unit) {
    ...
    val filtered = when (filter) { CASH -> entries.filter { it.account == CASH || it.fromAccount == CASH || it.toAccount == CASH }; BANK1 -> entries.filter { it.account == BANK1 || it.fromAccount == BANK1 || it.toAccount == BANK1 }; BANK2 -> entries.filter { it.account == BANK2 || it.fromAccount == BANK2 || it.toAccount == BANK2 }; else -> entries }
    LazyColumn(Modifier.fillMaxSize().padding(padding).padding(horizontal = 16.dp), verticalArrangement = Arrangement.spacedBy(8.dp), contentPadding = PaddingValues(vertical = 16.dp)) {
        item {
            ...
            Row(Modifier.horizontalScroll(rememberScrollState()).padding(vertical = 10.dp), horizontalArrangement = Arrangement.spacedBy(6.dp)) {
                listOf("Semua", CASH, BANK1, BANK2).forEach { FilterChip(filter == it, { filter = it }, label = { Text(it) }) }
            }
        }
