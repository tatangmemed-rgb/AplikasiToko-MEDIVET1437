...
// v0.15: 3 sumber saldo + transfer + backup/restore bukti yang aman. Data lama otomatis dianggap Kas Toko.
data class Entry(
    val id: Long,
    val date: String,
    val type: String,
...
private fun TransactionPage(
    ...
    val baseFiltered = when (filter) {
        ...
        "Pengeluaran" -> entries.filter { it.type == "Pengeluaran" }
    ...
    val filtered = if (filter == "Pengeluaran") {
        val from = parseFlexibleDate(startDate)
        val to = parseFlexibleDate(endDate)
        baseFiltered.filter { entry ->
            val d = parseFlexibleDate(entry.date)
            (from == null || (d != null && !d.before(from))) &&
                (to == null || (d != null && !d.after(to)))
        }
    } else {
        baseFiltered
    }

    val expenseTotal = filtered.filter { it.type == "Pengeluaran" }.sumOf { it.amount }
    val expenseCount = filtered.count { it.type == "Pengeluaran" }
    val periodLabel = when {
        startDate.isBlank() && endDate.isBlank() -> "Semua waktu"
        startDate.isNotBlank() && endDate.isNotBlank() -> "$startDate – $endDate"
        startDate.isNotBlank() -> "Mulai $startDate"
        else -> "Sampai $endDate"
    }
    ...
    LazyColumn(
        ...
        item {
            ...
            Row(
                ...
                listOf("Semua", "Pengeluaran", "Nota Belum Terbayar", "Nota Terbayar", CASH, BANK1, BANK2).forEach {
                    FilterChip(
                        filter == it,
                        { filter = it; onFilterChanged(if (it == "Semua") "" else it) },
                        label = { Text(it) }
                    )
                }
            }
        ...
        // Khusus filter Pengeluaran: tampilkan evaluasi total + rentang waktu.
        if (filter == "Pengeluaran") {
            item {
                Card(
                    modifier = Modifier.fillMaxWidth(),
                    colors = CardDefaults.cardColors(containerColor = Color(0xFFF4E9EA))
                ) {
                    Column(Modifier.padding(16.dp), verticalArrangement = Arrangement.spacedBy(4.dp)) {
                        Text("TOTAL PENGELUARAN", fontSize = 12.sp, color = Color.Gray, fontWeight = FontWeight.Bold)
                        Text(
                        ...
                        Text("$expenseCount transaksi • $periodLabel", color = Color.Gray, fontSize = 12.sp)
                    }
                }
            }
            item {
                Text("Rentang waktu", fontWeight = FontWeight.Bold, fontSize = 15.sp)
                Text("Isi tanggal untuk mengecek pengeluaran pada periode tertentu.", color = Color.Gray, fontSize = 12.sp)
                Spacer(Modifier.height(6.dp))
                OutlinedTextField(
                    value = startDate,
                    onValueChange = { startDate = it },
                    modifier = Modifier.fillMaxWidth(),
                    label = { Text("Dari (dd/MM/yyyy)") },
                    singleLine = true,
                    trailingIcon = {
                        if (startDate.isNotBlank()) IconButton(onClick = { startDate = "" }) {
                            Icon(Icons.Default.Clear, "Hapus tanggal mulai")
                        }
                    }
                )
                ...
                OutlinedTextField(
                    ...
                    modifier = Modifier.fillMaxWidth(),
                    label = { Text("Sampai (dd/MM/yyyy)") },
                    singleLine = true,
                    trailingIcon = {
                        if (endDate.isNotBlank()) IconButton(onClick = { endDate = "" }) {
                            Icon(Icons.Default.Clear, "Hapus tanggal akhir")
                        }
                    }
                )
                Spacer(Modifier.height(8.dp))
                Row(
                    ...
                    OutlinedButton(onClick = {
                        ...
                        val today = SimpleDateFormat("dd/MM/yyyy", Locale("id")).format(now.time)
                        startDate = today
                        endDate = today
                    }) { Text("Hari ini") }
                    OutlinedButton(onClick = {
                        ...
                        startDate = SimpleDateFormat("dd/MM/yyyy", Locale("id")).format(start.time)
                        endDate = SimpleDateFormat("dd/MM/yyyy", Locale("id")).format(end.time)
                    }) { Text("7 hari") }
                    OutlinedButton(onClick = {
                        val end = Calendar.getInstance()
                    ...
                    OutlinedButton(onClick = {
                        ...
                        startDate = SimpleDateFormat("dd/MM/yyyy", Locale("id")).format(start.time)
                        endDate = SimpleDateFormat("dd/MM/yyyy", Locale("id")).format(now.time)
                    }) { Text("Bulan ini") }
                    OutlinedButton(onClick = { startDate = ""; endDate = "" }) { Text("Semua") }
                }
            }
        }
        ...
        if (filtered.isEmpty()) item {
            Card(Modifier.fillMaxWidth()) {
                Text(
                    when {
                        filter == "Pengeluaran" && (startDate.isNotBlank() || endDate.isNotBlank()) ->
                            "Tidak ada pengeluaran pada rentang waktu ini."
                        filter == "Pengeluaran" -> "Belum ada pengeluaran yang diinput."
                        filter == "Nota Belum Terbayar" -> "Tidak ada nota yang belum terbayar."
                        filter == "Nota Terbayar" -> "Belum ada nota terbayar."
                ...
                Column(Modifier.padding(14.dp)) {
                    ...
                    Row(Modifier.horizontalScroll(rememberScrollState())) {
                        if (entry.type == "Nota Masuk" && unpaid.any { it.id == entry.id }) {
                            Button(onClick = { onPayNote(entry) }) {
                                Icon(Icons.Default.CheckCircle, null); Spacer(Modifier.width(4.dp)); Text("Bayar Nota")
                            }
                        }
                        TextButton(onClick = { onEdit(entry) }) {
                            Icon(Icons.Default.Edit, null); Spacer(Modifier.width(4.dp)); Text("Koreksi")
                        }
                        if (entry.proofPath.isNotBlank() && File(entry.proofPath).exists()) TextButton(onClick = { onViewProof(entry.proofPath) }) {
                            Icon(Icons.Default.Image, null); Spacer(Modifier.width(4.dp)); Text("Bukti")
                        }
                        TextButton(onClick = { onDelete(entry) }) {
                            Icon(Icons.Default.DeleteOutline, null); Spacer(Modifier.width(4.dp)); Text("Hapus")
                        }
                    }
                }
            }
...
private fun parseFlexibleDate(value: String): Date? {
    ...
    for (pattern in formats) {
        runCatching {
            return SimpleDateFormat(pattern, Locale("id")).apply { isLenient = false }.parse(s)
        }
    }
    return null
}
...
private fun writeBackup(context: Context, uri: Uri, entries: List<Entry>, opening: Map<String, Long>) {
    context.contentResolver.openOutputStream(uri)?.use { output -> ZipOutputStream(output).use { zip ->
        // v0.15: nama file bukti di dalam backup selalu dibuat pendek dan stabil.
        // Ini mencegah nama "restored_UUID_..." bertambah panjang setiap restore.
        val proofArchiveNames = mutableMapOf<String, String>()
        var proofCounter = 0
        entries.forEach { e ->
            if (e.proofPath.isNotBlank()) {
                val file = File(e.proofPath)
                if (file.exists()) {
                    proofArchiveNames.putIfAbsent(file.absolutePath, "proof_${(++proofCounter).toString().padStart(4, '0')}.jpg")
                }
            }
        }
        ...
        val json = JSONArray()
        entries.forEach { e ->
            val copy = if (e.proofPath.isNotBlank()) {
                val archiveName = proofArchiveNames[File(e.proofPath).absolutePath]
                if (archiveName != null) e.copy(proofPath = File(proofDir(context), archiveName).absolutePath) else e.copy(proofPath = "")
            } else e
            json.put(entryToJson(copy))
        }
        // entry JSON hanya menyimpan nama pendek; path lokal akan dibangun saat restore.
        ...
        proofArchiveNames.forEach { (sourcePath, archiveName) ->
            val file = File(sourcePath)
            if (file.exists()) {
                zip.putNextEntry(ZipEntry("proofs/$archiveName"))
                file.inputStream().use { it.copyTo(zip) }
                zip.closeEntry()
            }
        }
        zip.putNextEntry(ZipEntry("backup_info.txt")); zip.write("MEDIVET1437 KEUANGAN TOKO BACKUP v3\n".toByteArray(Charsets.UTF_8)); zip.closeEntry()
...
private fun readBackup(context: Context, uri: Uri): RestoreData {
    val tempByName = mutableMapOf<String, ByteArray>()
    var entriesJson = "[]"
    var openingJson = "{}"
        ...
        ZipInputStream(input).use { zip ->
            ...
            while (e != null) {
                if (!e.isDirectory) {
                    ...
                    when {
                        safe == "entries.json" -> entriesJson = zip.readBytes().toString(Charsets.UTF_8)
                        safe == "opening_balances.json" -> openingJson = zip.readBytes().toString(Charsets.UTF_8)
                        safe.startsWith("proofs/") && safe.substringAfterLast('/').isNotBlank() && !safe.contains("..") -> {
                            // Simpan berdasarkan basename agar kompatibel dengan backup v0.13/v0.14,
                            // termasuk backup yang sudah memiliki nama restored_... yang sangat panjang.
                            tempByName[safe.substringAfterLast('/')] = zip.readBytes()
                        }
                    }
                }
                zip.closeEntry()
    ...
    val a = JSONArray(entriesJson)
    val restored = mutableListOf<Entry>()
    proofDir(context).mkdirs()

    for (i in 0 until a.length()) {
        val o = a.getJSONObject(i)
        val proofName = o.optString("proofName", "")
        var path = ""
        val proofBytes = tempByName[proofName]
        if (proofName.isNotBlank() && proofBytes != null) {
            // SELALU gunakan nama pendek. Jangan pernah menambahkan nama file lama
            // ke nama file hasil restore. Ini membuat restore berulang tetap aman.
            val extension = when {
                proofName.endsWith(".png", true) -> ".png"
                proofName.endsWith(".webp", true) -> ".webp"
                else -> ".jpg"
            }
            val shortName = "proof_${UUID.randomUUID().toString().replace("-", "").take(12)}$extension"
            val f = File(proofDir(context), shortName)
            f.writeBytes(proofBytes)
            path = f.absolutePath
        }
        restored += Entry(
            o.getLong("id"),
            ...
            o.getString("note"),
            path,
            o.optString("account", CASH),
            o.optString("fromAccount", ""),
            o.optString("toAccount", ""),
            o.optLong("linkedEntryId", 0L)
        )
    }

    val oj = JSONObject(openingJson)
    val opening = mapOf(
        CASH to oj.optLong(CASH, 0L),
        BANK1 to oj.optLong(BANK1, 0L),
        BANK2 to oj.optLong(BANK2, 0L)
    )
