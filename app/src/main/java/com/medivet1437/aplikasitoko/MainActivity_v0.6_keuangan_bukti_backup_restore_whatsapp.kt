...
private fun EntryDialog(initial: Entry?, proofPath: String, onChooseGallery: () -> Unit, onTakeCamera: () -> Unit, onRemoveProof: () -> Unit, onDismiss: () -> Unit, onSave: (Entry) -> Unit) {
    ...
    AlertDialog(onDismissRequest = onDismiss, title = { Text(if (initial == null) "Tambah Transaksi" else "Koreksi Data") }, text = {
        Column {
            ...
            OutlinedTextField(date, { date = it }, label = { Text("Tanggal") }, Modifier.fillMaxWidth())
            Spacer(Modifier.height(8.dp))
            OutlinedTextField(amount, { amount = it.filter(Char::isDigit) }, label = { Text("Nominal") }, Modifier.fillMaxWidth())
            Spacer(Modifier.height(8.dp))
            OutlinedTextField(note, { note = it }, label = { Text("Keterangan") }, Modifier.fillMaxWidth(), minLines = 2)
